...
def main(hydra_cfg):
    ...
    # hydra_cfg = compose("config.yaml", overrides=["user=${env:USER}"])    

    yaml_config = OmegaConf.to_yaml(hydra_cfg, resolve=True)
    cfg = EasyDict(yaml.load(yaml_config))
    assert(cfg.multitask.training_task_id != -1)
    ...
    for (ep_idx, tree) in trees["trees"].items():
        if get_task_label(h5py_file[f"data/ep_{ep_idx}"].attrs["task_id"]) != cfg.multitask.training_task_id:
            continue
    ...
    # dbscan_clustering = cluster.DBSCAN(eps=0.05).fit(X)
    ...
    for ep_idx in loc_dict:
        ...
        for (loc, label) in loc_dict[ep_idx]:
            ...
            # if ep_idx == 1:
            #     print(node.start_idx, node.end_idx, label)
    ...
    subtask_file_name = f"results/skill_data/{cfg.data.dataset_name}_SingleTask_{cfg.multitask.training_task_id}_subtasks_{modality_str}_{cfg.repr.z_dim}_{cfg.agglomoration.footprint}_{cfg.agglomoration.dist}_{cfg.agglomoration.segment_footprint}_K{K}_{cfg.agglomoration.affinity}.hdf5"
    ...
    # locs = []
    # for (ep_idx, start_idx, end_idx, label) in merge_nodes:
    #     subtasks[label].append([ep_idx, start_idx, end_idx])
    #     if ep_idx not in ep_subtasks_seq:
    #         ep_subtasks_seq[ep_idx] = []
    #     ep_subtasks_seq[ep_idx].append((start_idx, end_idx, label))

    #     locs.append(ep_idx)
    #     embedding = segment_footprint(start_idx, end_idx, embedding_h5py_f[f"data/ep_{ep_idx}/embedding"][()], args.segment_footprint)
    #     X.append(embedding.squeeze())

    # clustering = clustering.SpectralClustering(n_clusters=K,
    #                                            assign_labels="discretize",
    #                                            affinity=cfg.agglomoration.affinity).fit(X)
    ...
    # grp.create_dataset("ep_subtasks_seq", data=str(ep_subtasks_seq))

    # ep_strings = []
    # for ep_idx in ep_subtasks_seq:
    #     subtask_seq = ep_subtasks_seq[ep_idx]
    #     subtask_seq.sort(key=take_start_idx)

    #     ep_string = []
    #     for subtask in subtask_seq:
    #         ep_string.append(subtask[-1])
    #     ep_strings.append(ep_string)
    ...
    for i in range(len(ep_strings)):
        for j in range(len(ep_strings)):
            ...
            score += dist
            num_pairs += 1
            # print(ep_strings[i], ep_strings[j], dist)
