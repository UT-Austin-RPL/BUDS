...
class HierarchicalAgglomorativeTree():
    ...
    def find_midlevel_abstraction(self, parent_node_idx, depth=0, no_leaf=False, min_len=30):
        # Return when depth = 0
        node_list = []
        for node_idx in self.nodes[parent_node_idx].children_node_indices:
            if depth == 0 and self.nodes[node_idx].len < min_len:
                node_list.append(node_idx)
            else:
                if no_leaf:
                    if self.nodes[node_idx].level == 1:
        ...
        while len(nodes) > 2:
            ...
            for i in range(len(nodes) - 1):
                ...
                if len_penalty:
                    ...
                    dist += (nodes[i].len + nodes[i+1].len) / (10. * len(nodes))
...
def save_agglomorative_tree(agglomorative_tree, agentview_image_names_list, ep_idx, dataset_name, footprint_mode, dist_mode, modality_mode):
        ...
        # cv2.waitKey(0)
        # print(image.shape)
        os.makedirs(f"results/{dataset_name}/agglomoration_results/{footprint_mode}_{dist_mode}_{modality_mode}", exist_ok=True)
        cv2.imwrite(f"results/{dataset_name}/agglomoration_results/{footprint_mode}_{dist_mode}_{modality_mode}/{dataset_name}_{ep_idx}.png", image)
